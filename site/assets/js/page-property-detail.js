...
function notFoundBlock() {
    ...
    return '<section class="bg-slate-50 min-h-[60vh] flex items-center">' +
        ...
        '<div class="text-sm font-semibold text-amber-600 uppercase tracking-wider mb-3">404 · Listing</div>' +
...
function detailHero(p) {
    ...
    return '<section class="relative bg-slate-900 overflow-hidden" data-hero-reveal>' +
        ...
        '<span class="text-[10px] font-bold uppercase tracking-wider bg-amber-500 text-slate-900 px-2.5 py-1 rounded-md">' + tag + '</span>' +
        (txnLabel ? '<span class="text-[10px] font-bold uppercase tracking-wider ' + (txn === 'rent' ? 'bg-slate-800 text-white' : 'bg-white text-slate-900') + ' px-2.5 py-1 rounded-md">' + txnLabel + '</span>' : '') +
        (p.placeholder ? '<span class="text-[10px] font-bold uppercase tracking-wider bg-slate-800/80 backdrop-blur text-slate-200 px-2.5 py-1 rounded-md">' + T.featured.sample + '</span>' : '') +
        '</span></div>' +
...
function descriptionSection(p) {
    ...
    return '<section class="py-16 lg:py-24 bg-white">' +
        ...
        '<div class="text-sm font-semibold text-amber-600 uppercase tracking-wider" data-reveal-up>' + T.properties.detail.description + '</div>' +
        '</div>' +
...
function locationSection(p) {
    ...
    return '<section class="py-16 lg:py-24 bg-slate-50">' +
        ...
        '<div><div class="text-sm font-semibold text-amber-600 uppercase tracking-wider mb-3" data-reveal-up>' + T.properties.detail.location + '</div>' +
...
function inquireSection(p) {
    ...
    return '<section class="py-16 lg:py-24 bg-white">' +
        ...
        '<div class="text-sm font-semibold text-amber-600 uppercase tracking-wider mb-3" data-reveal-up>' + T.cta.contact + '</div>' +
        ...
        '<a href="https://wa.me/' + WHATSAPP_RAW + '?text=' + encodeURIComponent(waMsg) + '" target="_blank" rel="noopener" class="flex items-center justify-between p-5 bg-emerald-500 hover:bg-emerald-400 text-white rounded-xl shadow-sm transition-colors" data-reveal-up>' +
        ...
        '<form data-reveal-up class="bg-slate-50 border border-slate-100 rounded-xl p-7 lg:p-9" onsubmit="return submitPropertyInquiry(event, \'' + p.id + '\', ' + JSON.stringify(subj).replace(/"/g, '&quot;') + ')">' +
...
function similarListings(current) {
    ...
    return '<section class="py-16 lg:py-24 bg-slate-50 border-t border-slate-200">' +
        '<div class="max-w-[1280px] mx-auto px-6 lg:px-10">' +
        '<div class="mb-8">' +
        '<div class="text-sm font-semibold text-amber-600 uppercase tracking-wider mb-3" data-reveal-up>' + T.properties.detail.similar + '</div>' +
