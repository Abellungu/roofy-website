...
(function () {
    ...
    function propertyCard(p, T) {
        ...
        return '<a href="/properties/detail.html?id=' + encodeURIComponent(p.id) + '" ' +
            'class="group block bg-white border border-slate-200 overflow-hidden transition-shadow duration-300" data-reveal-up>' +
            ...
            '</div>' +
            '<h3 class="font-display text-lg font-semibold text-slate-900 mb-3 leading-snug">' + title + '</h3>' +
            ...
            '<div class="text-amber-600 font-display font-semibold text-xl leading-none">' + p.price + '</div></div>' +
